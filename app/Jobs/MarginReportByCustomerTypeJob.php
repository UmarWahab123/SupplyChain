...
class MarginReportByCustomerTypeJob implements ShouldQueue
{
    ...
    public function handle()
    {
        try{
            ...
            $not_visible_arr=[];
            $not_visible_columns = TableHideColumn::select('hide_columns')->where('type','margin_report_by_customer_type')->where('user_id',$user_id)->first();
            ...
            //     }
            //     $this->dataArray[] = [
            //         'product_id' => $row->customer_type_id,
            //         'office' => $row->title,
            ...
            $return=\Excel::store(new MarginReportByOfficeExport($products,$not_visible_arr, $global_terminologies, $request), 'Margin-Report-By-Customer-Type.xlsx');

            if($return)
            {
                ExportStatus::where('type','margin_report_by_customer_type')->update(['status'=>0,'last_downloaded'=>date('Y-m-d H:i:s')]);
    ...
    {
      ExportStatus::where('type','margin_report_by_customer_type')->update(['status'=>2,'exception'=>$exception->getMessage()]);
      $failedJobException=new FailedJobException();
      $failedJobException->type="margin_report_by_customer_type";
