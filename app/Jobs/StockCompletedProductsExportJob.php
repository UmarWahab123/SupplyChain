...
use Carbon\Carbon;
use Excel;
use App\Exports\CompletedStockProductsExport;
use App\Variable;
use App\ExportStatus;
use App\FailedJobException;
class StockCompletedProductsExportJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try{
            ...
            $return= \Excel::store(new CompletedStockProductsExport($data,$global_terminologies),'Completed-Stock-Adjustment-'.$user_id.'-'.$timestamp.'.xlsx');
            if($return)
            {
                $status=ExportStatus::where('type','stock_completed_products_download')->where('user_id',$this->user_id)->first();
                $status->status=0;
    ...
    public function failed($exception)
    {
        ...
        ExportStatus::where('type','stock_completed_products_download')->where('user_id',$this->user_id)->update(['status'=>2,'exception'=>$exception->getMessage()]);
        $failedJobException=new FailedJobException();
        $failedJobException->type="Stock Completed Products Download";
