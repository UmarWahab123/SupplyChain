...
class MarginReportByProductType2Job implements ShouldQueue
{
    ...
    public function handle()
    {
        try{
            ...
            if($return)
            {
                ExportStatus::where('type','margin_report_by_product_type_2')->update(['status'=>0,'last_downloaded'=>date('Y-m-d H:i:s')]);
    ...
    {
      ExportStatus::where('type','margin_report_by_product_type_2')->update(['status'=>2,'exception'=>$exception->getMessage()]);
      $failedJobException=new FailedJobException();
      $failedJobException->type="margin_report_by_product_type_2";
