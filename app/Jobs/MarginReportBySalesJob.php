...
class MarginReportBySalesJob implements ShouldQueue
{
    ...
    public function handle()
    {
        try{
            ...
            $not_visible_arr=[];
            $not_visible_columns = TableHideColumn::select('hide_columns')->where('type','margin_report_by_sales')->where('user_id',$user_id)->first();
            ...
            $return=\Excel::store(new MarginReportByOfficeExport($products,$not_visible_arr, $global_terminologies, $request), 'Margin-Report-By-Sales.xlsx');

            if($return)
            {
                ExportStatus::where('type','margin_report_by_sales')->update(['status'=>0,'last_downloaded'=>date('Y-m-d H:i:s')]);
    ...
    {
      ExportStatus::where('type','margin_report_by_sales')->update(['status'=>2,'exception'=>$exception->getMessage()]);
      $failedJobException=new FailedJobException();
      $failedJobException->type="margin_report_by_sales";
