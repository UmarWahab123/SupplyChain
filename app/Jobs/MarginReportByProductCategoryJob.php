...
class MarginReportByProductCategoryJob implements ShouldQueue
{
     ...
     public function handle()
    {
        try{
            ...
            $not_visible_arr=[];
            $not_visible_columns = TableHideColumn::select('hide_columns')->where('type','margin_report_by_product_category')->where('user_id',$user_id)->first();
            ...
            // {
            // foreach ($rows as $row) {
            //     $stock = (new ProductCategory)->get_manual_adjustments_for_export($request,$row->category_id);
            ...
            $return=\Excel::store(new MarginReportByOfficeExport($products,$not_visible_arr, $global_terminologies, $request), 'Margin-Report-By-Product-Category.xlsx');

            if($return)
            {
                ExportStatus::where('type','margin_report_by_product_category')->update(['status'=>0,'last_downloaded'=>date('Y-m-d H:i:s')]);
    ...
    {
      ExportStatus::where('type','margin_report_by_product_category')->update(['status'=>2,'exception'=>$exception->getMessage()]);
      $failedJobException=new FailedJobException();
      $failedJobException->type="margin_report_by_product_category";
