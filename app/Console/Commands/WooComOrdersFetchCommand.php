...
class WooComOrdersFetchCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'woocom_orders:fetch';
     ...
     * @var string
     */
    protected $description = 'This Command fetches orders from Woo Commerce Website to Supply Chain ERP';
    ...
    public function handle()
    {
        ...
        $check_orders = \App\Models\Common\Order\Order::orderBy('woo_com_id','desc')->first();
        $orders = Order::all();
        // return 'checking ecom';
        DB::beginTransaction();
        foreach($orders as $order)
        {
            if ($order->id > $check_orders->woo_com_id) 
            { 
                ...
                else
                {
                 ...
                 $default_warehouse = $deployment->warehouse_id;
                ...
                foreach(@$order->line_items as $key => $value)
                {
                    ...
                    $o_products->woocom_item_id    = $value ->id;
                    ...
                    $wh_reserve_pro = WarehouseProduct::where('product_id',$o_products->product_id)->where('warehouse_id', $default_warehouse)->first();
                    if($product->ecom_selling_unit){
