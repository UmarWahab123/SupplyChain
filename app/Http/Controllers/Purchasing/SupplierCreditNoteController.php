...
class SupplierCreditNoteController extends Controller
{
    ...
    {
        $status     = Status::where('id', 25)->first();
        ...
        $c_p_ref = PurchaseOrder::where('primary_status', 25)->orderby('id', 'DESC')->first();
        $str = @$c_p_ref->ref_id;
        ...
        // dd($system_gen_no);
        $credit_note = PurchaseOrder::create(['created_by' => Auth::user()->id, 'status' => 26, 'ref_id' => $system_gen_no]);
        $po = PurchaseOrder::find($credit_note->id);
        $po->primary_status = 25;
        $po->save();
        return redirect()->route("get-supplier-credit-note-detail", $po->id);
    }
    ...
    public function getSupplierNoteDetail($id)
    {
        ...
        // dd($getPurchaseOrder);
        return view('accounting.notes.supplier-credit-note-detail',compact('getPurchaseOrderDetail','table_hide_columns','display_purchase_list','id','getPurchaseOrder','checkPoDocs','getPoNote','po_setting','company_info','warehouses','supplier_currency_logo','paymentTerms','columns_prefrences','hidden_columns','pos_count','hidden_columns_by_admin','user_plus_admin_hidden_columns','dummy_data','targetShipDate', 'suppliers'));
    }
