      ...
      <table class="table entriestable table-bordered table-quotation-product text-center" style="table-layout: fixed !important;">
        <thead>
          <tr>
            ...
            <th @if(in_array(22,$hidden_columns_by_admin)) class="noVis" @endif>Discount <br><i class="fa fa-plus" title="Add same discount on all items" data-toggle="modal" data-target="#addDiscountOnAllItems"></i></th>
...
// return;
  ...
  $(document).on('click', '.add-discount-on-all-items', function(e){
    ...
    $.ajax({
        method: "get",
        url: "{{route('add-discount-on-all-items')}}",
        data: 'id='+id+'&page=quotation&discount='+discount,
        beforeSend: function() {
            $("#loader_modal").modal('show');
